  ...
  const handleGenerate = async (image: string, textPrompt: string) => {
    ...
    try {
      ...
      if (data.html) {
        ...
        setAgentLogs(prev => [...prev, '❌ Error generating application.']);
      }
    } catch (err) {
      console.error(err);
      setAgentLogs(prev => [...prev, '❌ Error generating application.']);
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-white p-4">
      <header className="mb-6 flex items-center justify-between">
        ...
        <div className="text-sm text-neutral-400">Powered by Gemini Multimodal Vision</div>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-[calc(100vh-100px)]">
        ...
        <div className="flex flex-col h-full bg-neutral-900 rounded-xl border border-neutral-800 overflow-hidden">
          <DrawingCanvas onGenerate={handleGenerate} isGenerating={isGenerating} />
        </div>
        
        {/* Right column: Preview & Code & Agent */}
        <div className="flex flex-col h-full bg-neutral-900 rounded-xl border border-neutral-800 overflow-hidden">
          <PreviewFrame generatedHtml={generatedHtml} logs={agentLogs} isGenerating={isGenerating} />
        </div>
      </div>
    </div>
  );
